...
:::{#cr-vid1}
{{< video https://www.youtube.com/watch?v=kuvKukJNmzI width="500" height="350">}}
...
(Why? I am an unemployed statistician and thus in short supply of mysteries to tackle with my skills)

:::{#cr-vid2}
{{< video https://www.youtube.com/watch?v=uL5HALThICw width="500" height="350">}}
...
data_rel%>%
  ...
  geom_line()+
  scale_y_continuous("Bounty of Countries with few Medals among all Medals",labels = scales::percent,minor_breaks = NULL)+
...
Just look at its crazy 1980 result where it "brought home" (The Soviet Union hosted these games) almost twice as much (by percentage) Gold as Bronze.
...
status_two_years<-data%>%
  ...
  summarize(
    ...
    status=case_when(
      ...
      TRUE~"No Medals in previous Appearances"
    )
  )%>%
  ungroup()
...
two_years_cleaned%>%
  mutate(
    legacy=status%>%factor(levels = c("Already won Gold","Previously won Silver","Previously won Bronze", "No Medals in previous Appearances","First Olympics")),
    grouping=factor(grouping,levels=c("Europe","USA","Soviet Union","China"))
  )%>%
  ...
  scale_fill_manual("Who wins Gold?\n(Highest previous Result)",values=c(
    "First Olympics"="pink2","No Medals in previous Appearances"="green3","Previously won Silver"="#a7a7a7", "Previously won Bronze"="#a77044","Already won Gold"="#d6af36"
  ))+  
  theme(
    legend.position = "bottom", plot.background=element_rect(fill="#FFFFE0")
    )+
...
two_years_cleaned_all<-status_two_years_all%>%
  ...
  mutate(
    legacy=status%>%factor(levels = c("Previously won Gold","Previously won Silver","Previously won Bronze", "No Medals in previous Appearances","First Olympics")),
    grouping=factor(grouping,levels=c("Europe","USA","Soviet Union","China")),
    medal=factor(medal,levels=c("Gold","Silver","Bronze","None"))
  )

two_years_cleaned_all%>%
